...
\begin{center}
	...
	\begin{boxedminipage}[][18cm]{\linewidth}
		\begin{spacing}{1.5}
			...
			本文依据要求，依次完成了\nameref{cha:三点式正弦振荡器}、
			\nameref{cha:高频小信号调谐放大器实验}、
			\nameref{cha:模拟乘法混频}、
			\nameref{cha:高频小信号调谐放大器实验}、
			\nameref{cha:非线性丙类功率放大器实验}、
			\nameref{cha:模拟乘法器调幅及同步检波实验}、
			\nameref{cha:半双工调频无线对讲机}6个实验。对实验数据进
	...
	\begin{boxedminipage}[][18cm]{\linewidth}
		\begin{spacing}{1.5}
			...
			\begin{tabu} to \linewidth{@{}X[l]@{}X[l, 8]@{}}
				\textbf{Title} & \hspace{2em}\underline{\makebox[15em][c]{\zihao{4}\songti Experiment of High Frequency Electronic Circuit}} \\
				\phantom{Title} & \hspace{2em}\underline{\makebox[15em][c]{\zihao{4}\songti }} \\
			\end{tabu}
			...
			According to the requirements, the experiments of
			three-point sinusoidal oscillator and high-frequency
			small signal tuning amplifier are completed successively
			Quasi multiplicative mixing, high frequency small signal
			tuning amplifier experiment, nonlinear class C power
			amplifier experiment, analog multiplication There are
			six experiments of amplitude modulation and synchronous
			detection, half duplex FM radio. The experimental data
			were analyzed Reliable analysis. Verified the accuracy
			of the relevant conclusions, and obtained convincing
			results.

			\vspace{2em}

			\noindent\textbf{Keywords}\hspace{1em}High Frequency Electronic Circuit
